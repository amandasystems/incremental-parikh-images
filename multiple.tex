...
transition~$\Transition$ since $B$ only counts length. The reader is
...
\cref{ex:string-constraints} across the automaton $A'$ still apply for the
product. Note that this fixes the values of all transitions but one of $A'$ as
...
we obtain the version of $B$ shown in \cref{fig:aut_b_annotated} and represented
...
\begin{prooftree}
  ...
  \infer1[\Materialise]{
    \begin{array}{rrrr}
      ...
      \Image{}_{\Tuple{A',B}, \Map}(\Filter, \VectorLiteral{l_A, 1, 2, l_A}) 
    \end{array}
  }
  }% END RIGHT TREE
