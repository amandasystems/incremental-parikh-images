...
\begin{prooftree}
  % LEFT TREE  ->
  \hypo{l}% here x_12 is zero
  % RIGHT TREE ->
  \hypo{r} % here x_12 is one
  \infer2[\Split{} $\TransitionVar_{12} = 0$]{\text{FINAL STEP OF STUMP}}
  \infer1[\EquationReasoning: probably something good]{
    \begin{aligned}
      l_A = \sum_{k = 6}^{k = 12} \TransitionVar_k\\
      \land \TransitionVar_2 = l_A - 3\\
      \land 1 + \TransitionVar_{12}   = \TransitionVar_{11} + \TransitionVar_{12}\\
      \land \TransitionVar_{11} = \TransitionVar_6\\
      \land \TransitionVar_{12} = \TransitionVar_8 + \TransitionVar_{10}\\
