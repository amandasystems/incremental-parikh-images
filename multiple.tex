       ...
       {\begin{array}{c@{}}
           \left\{
            \begin{array}{c@{}}
              ...
              \Connected(\Automaton_i, \Filter),\\[1ex]
...
For precisely one automaton, neither rule applies, and we perform
...
product. Note that this fixes the values of all transitions but one of $\mathcal{A}'$, including the free counter variables $i, n$, whose new constant
...
instance but might lead to a larger product being computed, so we pick \Split{}. This leads to an opportunity for propagation since
...
branch; the use of the upper path to state~$A$ does not preclude visiting
state~$B$.

In both cases the automata are now simpler, so we apply
...
will lead to a flow equation $1 = 0$. We can then close the proof and know that the problem
