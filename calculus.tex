...
\begin{prooftree}
  ...
  \infer1[\EquationReasoning{}: $=$-elimination]{
    ...
    \Connected(A', \Filter) \land \\
    i = \TransitionVar_5 \land 
    n = \TransitionVar_4 + \TransitionVar_1 \land n \geq i \land l_A - i - n > 0 \\
    \land \TransitionVar_4 = \TransitionVar_1 
    \land \TransitionVar_2 = l_A - i - n 
    \land \TransitionVar_3 = 1 - \TransitionVar_1 
    \end{aligned}
  ...
  \infer1[\EquationReasoning{}: expand sum]{
    \begin{aligned}
        i = \TransitionVar_5, l_A = \sum_{k = 1}^{k = 5} \TransitionVar_k \land n = \TransitionVar_4 + \TransitionVar_1 \land n \geq i \land l_A - i - n > 0
...
automaton $A'$ of \cref{fig:aut_a}. Note that the flow equations of
\cref{eq:single-filter} are left out of the tree in order to keep it
manageable.}\label{fig:derivation:single}
\end{figure}
