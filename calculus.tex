...
conclusion~$\SomeClause$. When constructing a proof, we start with some
...
A proof in which all proof goals are closed witnesses that the formulas
...
\begin{equation}\label{eq:single-filter}
  \begin{array}{rl@{\qquad}rl@{\qquad}rl}
    \Filter(\FromLabelTo{D}{d/\begin{bmatrix}l_{A+}, n_+\end{bmatrix}}{F_A})
    &= \TransitionVar_1
    &
    \Filter(\FromLabelTo{F_A}{\Sigma/\begin{bmatrix} l_{A+} \end{bmatrix}}{F_A} )
    &= \TransitionVar_2   
    &
    \Filter(\FromLabelTo{I_A}{c/\begin{bmatrix}l_{A+}, n_+\end{bmatrix}}{F_A} ) 
    &=  \TransitionVar_3 
    \\
    \Filter(\FromLabelTo{I_A}{d/\begin{bmatrix}l_{A+}, n_+\end{bmatrix}}{D}) 
    &= \TransitionVar_4
    &
    \Filter(\FromLabelTo{I_A}{\Sigma/\begin{bmatrix} l_{A+} i_+\end{bmatrix}}{I_A}) 
    &= \TransitionVar_5  \\[3ex]
%
  \multicolumn{6}{c}{
...
Like in \cref{sec:a_1,sec:intuition:algebra}, we apply reasoning by equivalence
  ...
  Then, we remove the $\Connected{}$ predicate using \Subsume{} since no
...
\begin{prooftree}
  ...
  \infer1[\EquationReasoning{}: expand sum]{
    ...
    \left[l_A, i, n\right] =
    \sum\limits_{\Transition \in \Transitions_{\mathcal{A}'} } \Map(\Transition) \cdot \Filter(\Transition)
  }
  \infer1[\Expand{}]{
