...
\begin{definition}\label{def:connected} $\Connected(\Automaton, \Filter)$
  ...
  if $\Filter(\Transition) > 0$ then there is some $\Filter$-selected
  path that reaches $\Transition$'s starting state~$\State$. More formally,
  ...
  \StatesOf(p)$. Predicate~$\Connected(\Automaton, \Filter)$ represents the
